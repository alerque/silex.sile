...
local function forLanguage (langbcp47, tableOrCallback)
   ...
   if type(tableOrCallback) == "table" then
      matcher = function (lang)
         return tableOrCallback[lang]
      end
...
SILE.languageSupport.loadLanguage = function (language)
   ...
   if not langresource then
      SU.warn(("Unable to load language feature support (e.g. hyphenation rules) for %s"):format(language))
   else
      SU.debug("silex", ("Loaded language feature support for %s: matched %s"):format(language, matchedlang))
   ...
   if not ftlresource then
      SU.warn(("Unable to load localized strings (e.g. table of contents header text) for %s"):format(language))
   else
      SU.debug("silex", ("Load localized strings for %s from %s"):format(language, resolved_resource))
   end
